...
const LS_PROFILE="mm4_profile_v3";
const LS_STATS="mm4_stats_v3";
...
function scorePosition(b, player, style="balanced"){
  ...
  for(let r=0;r<ROWS;r++) for(let c=0;c<COLS-3;c++) s+=evalWindow([b[r][c],b[r][c+1],b[r][c+2],b[r][c+3]], player);
  for(let c=0;c<COLS;c++) for(let r=0;r<ROWS-3;r++) s+=evalWindow([b[r][c],b[r+1][c],b[r+2][c],b[r+3][c]], player);
  for(let r=0;r<ROWS-3;r++) for(let c=0;c<COLS-3;c++) s+=evalWindow([b[r][c],b[r+1][c+1],b[r+2][c+2],b[r+3][c+3]], player);
  for(let r=3;r<ROWS;r++) for(let c=0;c<COLS-3;c++) s+=evalWindow([b[r][c],b[r-1][c+1],b[r-2][c+2],b[r-3][c+3]], player);
  if(style==="aggressive") s*=1.08; else if(style==="defensive") s*=0.98; return s;
}
function minimax(b,d,maxing,style,bias,alpha=-Infinity,beta=Infinity){
  const w = winner(b);
  if(d===0 || w){ if(w===AI) return{score:1e9}; if(w===HUMAN) return{score:-1e9}; if(w===3) return{score:0}; return{score:scorePosition(b,AI,style)}; }
...
function updateStats(s,res){ const n={...s}; n.games++; if(res==='W'){n.wins++; n.curStreak++; n.bestStreak=Math.max(n.bestStreak,n.curStreak); n.rating+=12+Math.max(0,6-Math.floor(n.curStreak/2));}
...
/* ---------- confetti (z-index under overlay) ---------- */
function fireConfetti(ms=1200){
  ...
  const ps=Array.from({length:120},()=>({x:Math.random()*c.width, y:-20-Math.random()*c.height*.4, vx:(Math.random()-.5)*5, vy:3+Math.random()*4, g:.18+Math.random()*.18, w:8+Math.random()*6, h:12+Math.random()*8, a:Math.random()*Math.PI, s:(Math.random()<.5?-1:1)*(.1+Math.random()*.2), color:cols[Math.floor(Math.random()*cols.length)]}));
...
export default function App(){
  ...
  },[turn,over,board,profile,query]);

  // status
  useEffect(()=>{
    if (w===HUMAN) setStatus("You win! 🎉");
    else if (w===AI) setStatus("AI wins 🤖");
  ...
  useEffect(()=>{
    if (!over) return;
    const res = w===HUMAN? 'W' : w===AI? 'L' : 'D';
    const p = {...profile, lastTen:[...profile.lastTen,res].slice(-10)};
    const ns = updateStats(stats, res);
    adapt(p, ns); saveProfile(p); saveStats(ns); setProfile(p); setStats(ns);
    if (res==='W') fireConfetti(1200);
    // Show result overlay ABOVE confetti
    setOverlay(res==='W'?'win':res==='L'?'lose':'draw');
  ...
  function decide(b,p,q){
    ...
    // difficulty (+ challenge overrides)
  ...
  function shareUrl(){
    ...
    u.searchParams.set("mm4","1");
    u.searchParams.set("mode","streak");
    u.searchParams.set("target", String(stats.bestStreak||1));
    u.searchParams.set("depth", String(profile.aiConfig.depth));
    u.searchParams.set("rand", String(profile.aiConfig.randomness));
    u.searchParams.set("style", String(profile.aiConfig.style));
    return u.toString();
  }
  async function share(){
    const link = shareUrl();
    const text = `MindMatch 4 — beat my ${stats.bestStreak}-win streak!`;
    if (navigator.share){ try{ await navigator.share({title:"MindMatch 4 — Challenge", text, url:link}); return; }catch{} }
    await navigator.clipboard?.writeText(link); setToast("Challenge link copied!"); setTimeout(()=>setToast(null),1600);
  }
  ...
  return (
    <div style={st.page}>
      ...
      {/* Layout: board first (mobile), stats under; 2 columns on wide screens */}
      <main style={st.main}>
        {/* Board */}
        <section style={st.panel}>
          <div style={st.boardWrap}>
            <div style={st.boardFrame}>
              ...
              {/* visible holes + grid lines that work on all browsers */}
...
function Logo(){
  return (
    <svg width="36" height="36" viewBox="0 0 64 64" style={{marginRight:10}}>
      ...
      <text x="8" y="58" fontSize="14" fill="#e5e7eb" fontFamily="Arial">4</text>
    </svg>
  );
}
...
const st = {
  ...
  holes:{
    ...
    pointerEvents:"none",
    borderRadius:12,
    boxShadow:"inset 0 0 0 2px var(--grid), inset 0 6px 18px rgba(0,0,0,.35)"
  },
  ...
  disc:{width:"calc(var(--cell) - 10px)", height:"calc(var(--cell) - 10px)", borderRadius:"50%", transition:"transform .25s ease"},
  /* Stats */
...
/* Wide screens: put stats on the right */
const styleEl = document.createElement("style");
styleEl.textContent = `
@media (min-width: 900px){
  .mm4-main { grid-template-columns: 1fr 320px; }
}
`;
