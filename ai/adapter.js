...
(function () {
  ...
  function isColMajor(b) {
    return (
      Array.isArray(b) &&
      b.length === COLS &&
      b.every((col) => Array.isArray(col))
    );
  }
  function isGrid(b) {
    return (
  ...
  function sniffBoard() {
    ...
    if (cells.length) {
      ...
      cells.forEach((el) => {
        ...
        if (!Number.isNaN(r) && !Number.isNaN(c) && !Number.isNaN(v))
          grid[r][c] = v;
      });
  ...
  function setBoard(cols) {
    if (isColMajor(W.board)) {
      W.board = cols;
    } else if (W.board && isGrid(W.board)) {
      W.board = colsToGrid(cols);
    } else if (W.game && isColMajor(W.game.board)) {
      W.game.board = cols;
    } else if (W.game && isGrid(W.game.board)) {
      W.game.board = colsToGrid(cols);
    }
    if (typeof W.renderBoard === 'function')
      W.renderBoard(W.board || colsToGrid(cols));
  }
  ...
  function highlightCols(cols) {
    document
      .querySelectorAll('.col,.column,[data-col]')
